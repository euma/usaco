...
int main() {
  ...
  assert(data != NULL);
  i=0;
  while(fscanf(data,"%s",name[i]) != EOF) {
    i++;
  }
  names=i;
  printf("names==%d\n",names);
...
int search_name(char *name2) {
  ...
  while (1) {
    /*
    if (strcmp(name2,name[lb])==0) {
      return lb;
    }
    if (strcmp(name2,name[ub])==0) {
      return lb;
    }
    */
    ...
    printf("%d, %d, %d - %s\n", lb, m, ub, name[m]);
    //if (key < a[m]) {
    if (strcmp(name2,name[m]) < 0) {
      ub = m - 1;
    }
    else if (strcmp(name2,name[m]) > 0) {
    //else if (key > a[m]) {
      lb = m + 1;
    }
    else {
