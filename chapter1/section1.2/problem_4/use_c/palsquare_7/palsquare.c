...
int main() {
  ...
  int i,j;
  double log_max=log_base(N*N, 2);
  int max_size=(int)log_max+1;
  int buffer[max_size];

  double log_max_i=log_base(N, 2);
  ...
  for (i=1;i<=N;i++) {
    ...
    if (is_palindrome(buffer, buffer_size)) {
      int base_i=i;
      double log_base_i=log_base(base_i, base);
      int buffer_size_i=(int)log_base_i+1;
      convert_base(base_i, base, buffer_i, buffer_size_i);
